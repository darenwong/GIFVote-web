...
export function SQLProvider({children}){
  ...
  const updateDataset = (pollId) => {
    return new Promise((resolve, reject)=>{
      ...
      fetch(`${ENDPOINT}/api-vote-update/?user_id=${userId}&poll_id=${pollId}&isPersonal=${isPersonal}`)
      .then(res => res.json())
      .then(
        (results) => {
          console.log("success",results);
  ...
  const submitVote = ({user_id, poll_id, option_id}) => {
    ...
    fetch(`${ENDPOINT}/api-update/?user_id=${user_id}&poll_id=${poll_id}&option_id=${option_id}`, {method: 'POST'})
    .then((res) => {
      console.log(res);
      res.json();
    })
  ...
  const submitComment = ({user_id, poll_id, comment}) => {
    return new Promise((resolve, reject)=>{
      fetch(`${ENDPOINT}/api-insert-comments/?poll_id=${poll_id}&user_id=${user_id}&comment_text=${comment}`,     {
        method: 'POST'
      })
      .then(res => res.json())
      .then(
  ...
  const submitLike = ({poll_id, user_id}) => {
    return new Promise((resolve, reject)=>{
      fetch(`${ENDPOINT}/api-insert-like/?poll_id=${poll_id}&user_id=${user_id}`,     {
        method: 'POST'
      })
      .then(res => res.json())
      .then(
  ...
  const submitPoll = ({user_id, question, options}) => {
    return new Promise((resolve, reject)=>{
      fetch(`${ENDPOINT}/api-new-poll/?user_id=${user_id}&question=${question}&options=${options}`, 
      {
        method: 'POST'
      })
      .then((res) => {
  ...
  return (
    <SQLContext.Provider value={{data, getUserData, getDataset, updateDataset, refreshDataset, handleFetchMoreData, isPersonal, setIsPersonal, hasMore, userId, setSortBy, submitVote, getComments, submitComment, submitLike, submitPoll}}>
      {children}
    </SQLContext.Provider>
  )
}