...
function MyPoll({userId, isOpen, setIsOpen}) {
  ...
  useEffect(()=>{
    console.log("change",JSON.parse(JSON.stringify(data)), data.length);
  ...
  const getDataset = () => {
    ...
    fetch(`http://localhost:8080/api-vote/?user_id=${userId}&result=${result.current}&isPersonal=1`)
    .then(res => res.json())
    .then(
      (results) => {
  ...
  const updateDataset = (pollId) => {
    ...
    fetch(`http://localhost:8080/api-vote-update/?user_id=${userId}&poll_id=${pollId}&isPersonal=1`)
    .then(res => res.json())
    .then(
      (results) => {
        console.log("success",results);
  ...
  const getStyle = () => {
    if (isOpen) {return {transfrom:"translateX(100vw)", zIndex: "-1000"}}
    return {}
  }
        ...
        <InfiniteScroll
          ...
          <Grid container spacing={0} className={classes.gridContainer}>
            {data.map(({poll_text, gifURL, created_by, created_at, winner, chartData, voteData, poll_id, isVoted_bool, isVoted_option_id, totalVoteCount, comment_count, num_likes, user_liked}, index)=>{
              return (
                <Grid key={index} item xs={12} sm={6} lg={3} className={classes.itemContainer}>
